...
import { cacheWrapper, del, generateCacheKey } from './cache';
...
export async function addMessage(conversationId, role, content) {
  ...
  invalidateConversationCache(conversationId, conversation.userId);
 ...
 */
function invalidateConversationCache(conversationId, userId) {
...
function invalidateUserConversationsCache(userId) {
  // Use pattern to delete all user conversation caches for this user
  const keyPattern = `userConversations:userId:${userId}`;
  
  // Get all cache keys
  const keys = Object.keys(require('./cache').default.getStats().keys)

...
export const VoiceChatProvider: React.FC<VoiceChatProviderProps> = ({ children }) => {
  ...
  const { 
    ...
    onData: (audioBlob: Blob) => handleAudioProcessing(audioBlob),
    onStatusChange: (status: RecordingStatus, details?: any) => {
      console.log(`Recording status: ${status}`, details);
      if (status === 'error') {
        handleRecordingError(details);
      }
    },
    onError: (error: Error) => handleRecordingError({ message: error.message, type: 'unknown' }),
    debug: process.env.NODE_ENV === 'development'
  });
  
  // Handle recording errors with detailed information
  const handleRecordingError = (errorDetails: { message?: string; type?: string; details?: any }) => {
  ...
  const handleAudioProcessing = async (audioBlob: Blob) => {
    try {
      ...
      errorHandler.handleProcessingError(err, 'transcription');
      setIsProcessing(false);
    }
  };
  ...
  const handleSubmit = useCallback(async (text: string) => {
    ...
    try {
      ...
      const err = error instanceof Error ? error : new Error('Unknown error');
      errorHandler.handleProcessingError(err, 'text-input');
      setIsProcessing(false);
    }

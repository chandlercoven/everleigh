...
/**
 * Props interface for VoiceVisualizer component
 */
interface VoiceVisualizerProps {
  isActive?: boolean;
  audioLevel?: number;
  status?: 'idle' | 'requesting_permission' | 'recording' | 'processing' | 'error';
  theme?: 'light' | 'dark';
  size?: 'sm' | 'md' | 'lg';
}
...
const VoiceVisualizer: React.FC<VoiceVisualizerProps> = ({ 
  ...
  const generateBars = () => {
    if (!isActive) {
      ...
      return Array(barCount).fill(null).map((_, i) => (
        <div 
          key={i}
  ...
  return (
    <div className="voice-visualizer flex flex-col items-center">
      ...
      {/* Main visualizer */}
      <div className={`relative ${visualizerSize} flex items-center justify-center`} ref={visualizerRef}>
        {/* Circular background */}
        <div 
          className={`
